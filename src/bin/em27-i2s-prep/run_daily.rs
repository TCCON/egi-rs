...
use egi_rs::{config::DetectorSet, default_files, i2s_catalog::{self, make_catalog_entries}, utils::{ensure_trailing_path_sep, daily::render_daily_pattern}};
...
pub(crate) fn prep_daily_i2s(args: DailyCli) -> error_stack::Result<(), CliError> {
    ...
    while curr_date <= args.end_date {
        ...
        let n_entries = add_catalog_to_top(
            &mut i2s_input_file,
            &interferograms, &args.site_id,
            &args.common.coord_file_pattern,
            &args.common.met_file_pattern,
            curr_date
        ).change_context_lazy(|| CliError::IoError(
            format!("Error occurred while adding catalog to {}", i2s_input_path.display())
        ))?;
        debug!("{} interferograms written to the catalog in {}", n_entries, i2s_input_path.display());
...
fn setup_dirs(igram_pattern: &str, run_dir_pattern: &str, site_id: &str, curr_date: chrono::NaiveDate, clear_existing: bool)
...
fn create_i2s_top(igram_dir: &Path, run_dir: &Path, spec_dir: &Path,
                  interferograms: &[PathBuf], detectors: Option<DetectorSet>, site_id: &str, 
                  user_utc_offset: Option<&str>, top_file_template: Option<&Path>,
                  curr_date: chrono::NaiveDate) -> error_stack::Result<(std::fs::File, PathBuf), CliError> {
    ...
    let detectors = if let Some(det) = detectors {
        ...
        let dtmp = DetectorSet::infer_from_multi_headers(&interferograms)
        .change_context_lazy(|| CliError::BadInput(format!("Unable to infer detector set for {curr_date}")))?;
        log::info!("Interferograms on {curr_date} appear to use {dtmp} detector(s)");
        dtmp
    };

    let utc_offset = get_utc_offset(user_utc_offset, interferograms)
    ...
    let rel_spec_dir = spec_dir.strip_prefix(run_dir).expect("spec_dir should be a subdirectory of run_dir");
...
fn add_catalog_to_top(i2s_input_file: &mut std::fs::File, interferograms: &[PathBuf],
                      site_id: &str, coord_file_pattern: &str, met_file_pattern: &str,
                      curr_date: chrono::NaiveDate) -> error_stack::Result<usize, CliError> {
    ...
    let catalog_entries = make_catalog_entries(
        &coordinate_file, 
        &met_source_file, 
        &interferograms, 
        false
...
fn write_flimit_file(run_dir_path: &Path, detectors: DetectorSet) -> error_stack::Result<(), CliError> {
