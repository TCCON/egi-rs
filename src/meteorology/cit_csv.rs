...
impl TcconMetSite {
    fn add_timezone(
        ...
        match datetime.and_local_timezone(tz) {
            chrono::LocalResult::None => Err(DateTimeError::InvalidTimezone(format!(
                "{datetime} does not exist in time zone with UTC offset {utc_offset}"
            ))),
            chrono::LocalResult::Single(dt) => Ok(dt),
            chrono::LocalResult::Ambiguous(_, _) => Err(DateTimeError::InvalidTimezone(format!(
                "{datetime} has multiple representation in time zone with UTC offset {utc_offset}"
            ))),
        }
    }
}
...
pub(super) fn read_cit_csv_met(
    ...
    let temperature = if let Some(file) = temp_file {
        ...
        std::iter::repeat(CATALOG_FILL_FLOAT_F64)
            .take(pressure.len())
            .collect_vec()
    };
    ...
    for (i, time) in times.into_iter().enumerate() {
        let datetime =
            chrono::NaiveDateTime::parse_from_str(&time, "%Y-%m-%d %H:%M:%S").map_err(|e| {
                CitMetError::ParsingError {
                    file: pres_file.to_path_buf(),
                    line: i + 2,
                    col: 1,
                    reason: e.to_string(),
                }
...
fn read_cit_csv(
    csv_file: &Path,
    second_colname: &str,
) -> Result<(Vec<String>, Vec<f64>), CitMetError> {
        ...
        .collect_tuple()
    {
        if !col1.contains("Time") {
            return Err(CitMetError::UnexpectedColumn {
        ...
        if !col2.contains(second_colname) {
            return Err(CitMetError::UnexpectedColumn {
                file: csv_file.to_path_buf(),
                col_index: 2,
                expected: second_colname.to_string(),
            });
        }
    };
...
fn check_times(
    ...
    if main_times.len() != new_times.len() {
        return Err(CitMetError::TimeMismatch {
            ...
            cause: "different numbers of times".to_string(),
        });
    }
