...
pub(super) fn read_legacy_met_csv(csv_file: &Path, em27_tz: FixedOffset) -> error_stack::Result<Vec<MetEntry>, LegacyMetError> {
...
fn read_legacy_inner<R: std::io::Read>(input: R, em27_tz: FixedOffset) -> error_stack::Result<Vec<MetEntry>, LegacyMetError> {
    ...
    for (idx, row) in rdr.deserialize().enumerate() {
        ...
        let entry = raw.to_met_entry(em27_tz)
    ...
    fn to_met_entry(self, em27_tz: FixedOffset) -> Result<MetEntry, LegacyMetError> {
        let datetime = if let Some(timestamp) = self.CompSrlDate {
            ...
            dt.and_local_timezone(em27_tz).single().ok_or_else(|| {
                LegacyMetError::InvalidTime(format!("Matlab-style date number {timestamp} cannot be assigned time zone {em27_tz}"))
            })?
            ...
            return Err(LegacyMetError::InvalidTimeFormat(
                "none of CompSrlDate, CompDate + CompTime, or UTCDate + UTCTime were given".to_string()
            ));
        };
...
mod tests {
    ...
    fn test_matlab_datenum() {
        ...
        assert_eq!(dt, chrono::NaiveDateTime::parse_from_str("2014-09-12 20:10:15", "%Y-%m-%d %H:%M:%S").unwrap());
    }
    ...
    fn test_compsrl_file() {
        ...
        let mut entries = read_legacy_inner(wiki_example.as_bytes(), FixedOffset::west_opt(7*3600).unwrap())
            .unwrap()
            .into_iter();
        let entry = entries.next().unwrap();
        let dtime = chrono::DateTime::parse_from_rfc3339("2015-02-10T17:31:44-07:00").unwrap();
        assert!(entry.is_close(&MetEntry { datetime: dtime, temperature: Some(19.9), pressure: 985.9, humidity: Some(46.0) }));
